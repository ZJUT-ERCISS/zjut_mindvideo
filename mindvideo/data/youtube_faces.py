...
# ============================================================================
""" YouTube Faces dataset. TODO: finish these dataset API. """
...
__all__ = ["YoutubeFaces", "ParseYoutubeFaces"]


@ClassFactory.register(ModuleType.DATASET)
class YoutubeFaces(VideoDataset):
    """
    Args:
    ...
    Examples:
        >>> from mindvision.mindvideo.dataset.sports_1m import YoutubeFaces
        >>> dataset = YoutubeFaces("./data/","train")
        >>> dataset = dataset.run()
    ...
    def __init__(self,
        ...
        load_data = ParseYoutubeFaces(os.path.join(path, split)).parse_dataset
    ...
    def index2label(self):
        ...
        csv_file = os.path.join(self.path, f"YoutubeFaces_{self.split}.csv")
        mapping = []
    ...
    def download_dataset(self):
        """Download the YoutubeFaces data if it doesn't exist already."""
        raise ValueError("YoutubeFaces dataset download is not supported.")
...
class ParseYoutubeFaces(ParseDataset):
    """
    Parse YouTube Faces dataset.
    """

    def load_cls_file(self):
        """Parse the category file."""
        base_path = os.path.dirname(self.path)
        csv_file = os.path.join(base_path, f"YoutubeFaces_train.csv")
        cls2id = {}
        id2cls = []
    ...
    def parse_dataset(self, *args):
        """Traverse the YoutubeFaces dataset file to get the path and label."""
        parse_YoutubeFaces = ParseYoutubeFaces(self.path)
        split = os.path.split(parse_YoutubeFaces.path)[-1]
        video_label, video_path = [], []
        _, cls2id = self.load_cls_file()
        with open(os.path.join(os.path.dirname(parse_YoutubeFaces.path),
                               f"YoutubeFaces_{split}.csv"), "rt")as f:
            f_csv = csv.DictReader(f)
            for row in f_csv:
