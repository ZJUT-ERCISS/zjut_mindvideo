...
# ============================================================================
""" FBMS dataset. TODO: finish these dataset API. """
...
__all__ = ["FBMS", "ParseFBMS"]


@ClassFactory.register(ModuleType.DATASET)
class FBMS(VideoDataset):
    """
    Args:
    ...
    Examples:
        >>> from mindvision.mindvideo.dataset.sports_1m import FBMS
        >>> dataset = FBMS("./data/","train")
        >>> dataset = dataset.run()
    ...
    def __init__(self,
        ...
        load_data = ParseFBMS(os.path.join(path, split)).parse_dataset
    ...
    def index2label(self):
        ...
        csv_file = os.path.join(self.path, f"fbms_{self.split}.csv")
        mapping = []
    ...
    def download_dataset(self):
        """Download the FBMS data if it doesn't exist already."""
        raise ValueError("FBMS dataset download is not supported.")
...
class ParseFBMS(ParseDataset):
    ...
    def load_cls_file(self):
        ...
        csv_file = os.path.join(base_path, f"fbms_train.csv")
        cls2id = {}
        id2cls = []
    ...
    def parse_dataset(self, *args):
        """Traverse the FBMS dataset file to get the path and label."""
        parse_FBMS = ParseFBMS(self.path)
        split = os.path.split(parse_FBMS.path)[-1]
        video_label, video_path = [], []
        _, cls2id = self.load_cls_file()
        with open(os.path.join(os.path.dirname(parse_FBMS.path),
                               f"fbms_{split}.csv"), "rt")as f:
            f_csv = csv.DictReader(f)
            for row in f_csv:
