...
class DeformConv2d(nn.Cell):
    ...
    Args:
        ...
        pad_mode (str): Specifies padding mode. The optional values are "same", "valid" and "pad". Default: "same".
    ...
    def __init__(self, inc, outc, kernel_size=3, stride=1, pad_mode='same', padding=0, has_bias=False, modulation=True):
        ...
        self.p_conv = nn.Conv2d(inc, 2 * kernel_size * kernel_size, kernel_size=kernel_size,
                                pad_mode=pad_mode, padding=padding, stride=stride, has_bias=has_bias)
        ...
        if kernel_size % 2 == 0:
            raise ValueError(
                f"Only odd number is supported, but current kernel sizeis {kernel_size}")
