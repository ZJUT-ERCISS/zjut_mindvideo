...
[English](https://gitee.com/yanlq46462828/zjut_mindvideo)|中文

## 简介

MindSpore Video(msvideo) 是一个基于MindSpore的计算机视觉研究和开发的开源视频工具箱。它收集了一系列经典的和SoTA的视觉模型，如C3D和ARN，以及它们的预训练权重和训练策略。通过解耦模块的设计，很容易将msvideo应用于或适应你自己的CV任务。
...
MindSpore Video(msvideo) 支持的数据集可以从以下链接下载：
...
在`msvideo`中, 根据所使用的每个数据集的所有数据处理方法都可以在`data` 文件夹中找到。 

## 快速开始

### 运行

每个`msvideo`支持的模型都有两种方法用于训练、评估和推理。通过官方网站安装MindSpore后，可以在 `example` 文件夹下运行训练或评估文件，根据每个模型的名称，示例 "文件夹是一个专门为初学者设计的训练和评估的独立模块。另一种是在处理包含每个模型所需参数的`YAML`文件时，使用版本库根文件夹下所有模型的训练和推理接口，因为我们也支持一些参数配置以快速启动。对于这种方法，以I3D为例，只需运行以下命令进行训练。
...
同时, [paperswithcode](https://paperswithcode.com) 是浏览`msvideo`模型的良好资源，不同的模型都可以在以下链接找到:
...
- [x] [C3D](msvideo/example/arn/README.md) 动作识别

- [x] [I3D](msvideo/example/i3d/README.md) 动作识别

- [x] [X3D](msvideo/example/x3d/README.md) 动作识别

- [x] [R(2+1)d](msvideo/example/r(2+1)d/README.md) 动作识别

- [x] [NonLocal](msvideo/example/nonlocal/README.md) 动作识别

- [x] [ViST](msvideo/example/vist/README.md) 动作识别

- [x] [fairMOT](msvideo/example/fairmot/README.md) 单次学习跟踪

- [x] [VisTR](msvideo/example/vistr/README.md)实例分割

- [x] [ARN](msvideo/example/arn/README.md) 少样本动作识别
...
4. 通过浏览器打开 `_build/html/index.html` 
...
MindSpore Video(msvideo)是一个基于MindSpore的Python包，提供以下高级功能:
...
我们感谢所有对改善 MindSpore Video的贡献。请参考[CONTRIBUTING.md](CONTRIBUTING.md)的贡献指南
